...
<template>
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="DicTypeId">
              <a-input v-model="queryParam.dicTypeId" placeholder=""/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
    ...
    <a-table :dataSource="tableList" :pagination="pagination" @change="handleTableChange" :loading="loading" rowKey="id">
        ...
        <a-table-column title="DicTypeId" dataIndex="dicTypeId" />
        <a-table-column title="字典名称" dataIndex="name" />
        <a-table-column title="字典值" dataIndex="value" />
        <a-table-column title="排序" dataIndex="sort" />
          <a-table-column title="字典类型状态" dataIndex="status">
...
export default {
  ...
  methods: {
    getDataList(){
      this.loading = true;
      axios.get("api/Dicinfo",{params : this.queryParam}).then(res=>{
    ...
    handleDel(id){
        axios.delete('api/Dicinfo/' + id).then(res => {
