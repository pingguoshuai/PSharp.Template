...
namespace SchemaMapper
{
    ...
    public class Generator
    {
        ...
        private Entity CreateEntity(EntityContext entityContext, TableSchema tableSchema)
        {
            ...
            var tablearr = tableSchema.Name.Split('.');
            string tablePrefix = tableSchema.Name;
            if (tablearr.Length > 1)
            {
                tablePrefix = string.IsNullOrWhiteSpace(tablearr[1]) ? tableSchema.Name : tablearr[1];
                entity.TableName = tablePrefix;
                if (string.IsNullOrWhiteSpace(entity.TableSchema))
                {
                    entity.TableSchema = ToLegalName(tablearr[0]);
                }
            }
            string className = ToClassName(tablePrefix);
        ...
        private void CreateProperties(Entity entity, TableSchema tableSchema)
        {
            foreach (ColumnSchema columnSchema in tableSchema.Columns)
            {
                ...
                property.Description = string.IsNullOrWhiteSpace(columnSchema.Description)? columnSchema.Name : columnSchema.Description;

                property.DataType = columnSchema.DataType;

                property.SystemType = columnSchema.SystemType;

                if (_databaseSchema.Provider.Name == "MySQLSchemaProvider")
                {
                    if (columnSchema.DataType == DbType.SByte)
                    {
                        property.DataType = DbType.Boolean;
                        property.SystemType = typeof(bool);
                    }

                    if (columnSchema.NativeType == "char" && columnSchema.Size == 36)
                    {
                        property.DataType = DbType.Guid;
                        property.SystemType = typeof(Guid);
                    }
                }
