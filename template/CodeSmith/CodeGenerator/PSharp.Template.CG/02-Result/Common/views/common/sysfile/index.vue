...
<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="扩展名">
              <a-input v-model="queryParam.extension" placeholder=""/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
    ...
    <a-table :dataSource="tableList" :pagination="pagination" @change="handleTableChange" :loading="loading" rowKey="id">
        ...
        <a-table-column title="扩展名" dataIndex="extension" />
...
export default {
  ...
  methods: {
    getDataList(){
      this.loading = true;
      axios.get("api/Sysfile",{params : this.queryParam}).then(res=>{
    ...
    handleDel(id){
        axios.delete('api/Sysfile/' + id).then(res => {
