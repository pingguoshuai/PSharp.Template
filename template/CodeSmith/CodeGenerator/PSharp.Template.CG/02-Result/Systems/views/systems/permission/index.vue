...
<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="角色标识">
              <a-input v-model="queryParam.roleId" placeholder=""/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
            <a-form-item label="资源标识">
              <a-input v-model="queryParam.resourceId" placeholder=""/>
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
            <a-form-item label="拒绝">
              <a-select v-model="queryParam.isDeny" placeholder="请选择" default-value="">
    ...
    <a-table :dataSource="tableList" :pagination="pagination" @change="handleTableChange" :loading="loading" rowKey="id">
        ...
        <a-table-column title="角色标识" dataIndex="roleId" />
        <a-table-column title="资源标识" dataIndex="resourceId" />
          <a-table-column title="拒绝" dataIndex="isDeny">
            <template slot-scope="isDeny">
              <span>
                <a-tag v-if="isDeny" color="blue">是</a-tag>
...
export default {
  ...
  methods: {
    getDataList(){
      this.loading = true;
      axios.get("api/Permission",{params : this.queryParam}).then(res=>{
    ...
    handleDel(id){
        axios.delete('api/Permission/' + id).then(res => {
