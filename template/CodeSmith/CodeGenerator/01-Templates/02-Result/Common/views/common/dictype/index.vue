...
<template>
  <a-card :bordered="false">
    ...
    <a-table :dataSource="tableList" :pagination="pagination" @change="handleTableChange" :loading="loading" rowKey="id">
        ...
        <a-table-column title="类型名称" dataIndex="name" />
        <a-table-column title="类型代码" dataIndex="code" />
...
export default {
  ...
  methods: {
    getDataList(){
      this.loading = true;
      axios.get("api/Dictype",{params : this.queryParam}).then(res=>{
    ...
    handleDel(id){
        axios.delete('api/Dictype/' + id).then(res => {
