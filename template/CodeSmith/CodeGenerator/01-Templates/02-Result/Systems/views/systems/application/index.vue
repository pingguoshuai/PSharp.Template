...
<template>
  <a-card :bordered="false">
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="48">
          ...
          <a-col :md="8" :sm="24">
            <a-form-item label="启用注册">
              <a-select v-model="queryParam.registerEnabled" placeholder="请选择" default-value="">
                <a-select-option value="">全部</a-select-option>
                <a-select-option value="true">是</a-select-option>
                <a-select-option value="false">否</a-select-option>
              </a-select>
    ...
    <a-table :dataSource="tableList" :pagination="pagination" @change="handleTableChange" :loading="loading" rowKey="id">
        ...
        <a-table-column title="应用程序编码" dataIndex="code" />
          ...
          <a-table-column title="启用注册" dataIndex="registerEnabled">
            <template slot-scope="registerEnabled">
              <span>
                <a-tag v-if="registerEnabled" color="blue">是</a-tag>
                <a-tag v-else color="orange">否</a-tag>
              </span>
            </template>
          </a-table-column>
...
export default {
  ...
  methods: {
    getDataList(){
      this.loading = true;
      axios.get("api/Application",{params : this.queryParam}).then(res=>{
    ...
    handleDel(id){
        axios.delete('api/Application/' + id).then(res => {
